...
function DuplicationsPage(prop) {
  ...
  const [dataForDuplicationChart, setDataForDuplicationChart] = useState({
    labels: [],
    data: { duplication: [] },
  });
  ...
  useEffect(() => {
    ...
    if (currentProject != undefined) {
      let repositoryDTO = currentProject.repositoryDTOList.find(
        (x) => x.type == "sonar"
      );
  ...
  return (
    <div style={{ marginLeft: "10px" }}>
      ...
      <h2>
        <a href={duplicationUrl} target="blank">
          {
            dataForDuplicationChart.data.duplication[
              dataForDuplicationChart.data.duplication.length - 1
            ]
          }
          %
        </a>
      ...
      <div className={classes.root}>
        <div style={{ width: "67%" }}>
          <div>
            ...
            <div>
              <DrawingBoard
                data={dataForDuplicationChart}
                maxBoardY={100}
                id="duplications-chart"
              />
            </div>
          </div>
        </div>
