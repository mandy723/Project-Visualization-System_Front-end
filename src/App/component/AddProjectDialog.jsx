...
export default function AddProjectDialog({
  ...
  const createProject = () => {
    ...
    if (
      projectName === "" ||
      (githubRepositoryURL === "" && sonarRepositoryURL === "")
    ) {
      alert("不準啦馬的>///<");
    } else {
  ...
  const checkGithubRepositoryURL = () => {
    return Axios.get(
      `http://localhost:9100/pvs-api/repository/github/check?url=${githubRepositoryURL}`,
      { headers: { Authorization: `${jwtToken}` } }
    )
      .then((response) => {
  ...
  const checkSonarRepositoryURL = () => {
    return Axios.get(
      `http://localhost:9100/pvs-api/repository/sonar/check?url=${sonarRepositoryURL}`,
      { headers: { Authorization: `${jwtToken}` } }
    )
      .then((response) => {
  ...
  return (
    ...
    >
      ...
      <DialogContent>
        <DialogContentText>
          To create a project, please enter the project name and the repository
          URL you want to subscribe here.
        </DialogContentText>
        <TextField
          autoFocus
          margin="dense"
